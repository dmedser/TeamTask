...
case class Project(id: Long, name: String, customerId: Long)
...
class ProjectRepository @Inject()(dbapi: DBApi,
                                  customerRepository: CustomerRepository)(
    implicit ec: DatabaseExecutionContext) {

  private val db = dbapi.database("default")

  private val projectParser = {
    get[Long]("project.id") ~
  ...
  private val projectWithCustomerParser = projectParser ~ customerRepository.customerParser map {
    case project ~ customer ⇒ project → customer
  }

  def findById(id: Long): Future[Option[Project]] = Future {
    db.withConnection { implicit connection ⇒
      SQL"select * from project where id = $id".as(projectParser.singleOpt)
    }
  }
  ...
  def insert(project: Project): Future[Option[Long]] = Future {
    db.withConnection { implicit connection ⇒
      SQL"""
        ...
        ((select next value for project_seq), {name}, {customerId})
  ...
  def update(idOfProjectForUpdate: Long, project: Project) = Future {
    db.withConnection { implicit connection ⇒
      SQL"update project set name = {name}, customer_id = {customerId} where id = {id}"
        .bind(project.copy(id = idOfProjectForUpdate))
        .executeUpdate()
    }
  }
